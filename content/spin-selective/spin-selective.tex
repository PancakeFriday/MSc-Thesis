...
\todo{if rydberg dressing wasn't discussed in the Motivation, do it here}
...
\caption{Fraction of trap depths for the spin-tweezer and initial tweezers as a function of the spin-tweezer laser power. For the $F=1,m_F=1$ (purple) and $F=2,m_F=2$ (blue) state, only one state is trapped. Already at $\SI{2}{\milli\watt}$ power, the spin tweezer is more than 10 times deeper than the \ac{slm} tweezer. To argue that the bandwidth of the laser is not relevant for the efficiency of the spin-selective trapping, a bandwidth of $\SI{100}{\giga\hertz}$ was assumed and results in the shaded area surrounding the lines.}%
...
The laser classified in the following is a homebuilt model, the design of which is given in Appendix~\ref{aaaa}\todo{ref}. It has room for a laser diode, whose light is collimated, filtered and reflected through a partially reflective mirror, which also serves as the output of the housing.
...
The laser power is classified as a function of current supplied to the laserdiode. At the point where the device starts to lase, the power is linear with current and the diagram for the model built here is given in Figure~\ref{fig:laser_curve}.
